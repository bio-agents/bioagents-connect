########## Import of Bioconductor agents to bio.agents
## Issues:
...
map_terms <- function(agent) {
  views <- strsplit(agent["biocViews"],"\\|")
  tEDAM <- NULL
  for (i in views) {
  ...
  if (is.na(terms)) {
    ...
    for (e in unlist(edam_list)) {
      ...
      if (length(edam_name)>0) {
        if (nchar(e) <= 1) {
          EDAMTypos <<- rbind(EDAMTypos, c(currAgent$Name,e))
        } else {
          tnode4 <- newXMLNode("format",parent=parentnode)
...
## Writing xml-file
xml_out = newXMLNode(name="agents",namespace=list(xmlns="http://bio.agents"),
                     namespaceDefinitions = list("xsi"="http://www.w3.org/2001/XMLSchema-instance"),
                     attrs = list("xsi:schemaLocation"="http://bio.agents bioagents-2.0.0.xsd"))
for (i in 1:nrow(FullPcks)) {
  ...
  agentID <- gsub("\\!","",currAgent$Name)
  agentID <- gsub(" ","_",agentID)
  agentID <- gsub("\\+","Plus",agentID)
  agentID <- gsub("\\.","",agentID)
  agentID <- strtrim(agentID,12)
  newXMLNode("agentID",parent=tnode2,text=sub("\\(.*","",agentID))
  if (!is.na(currAgent["Version"]) & currAgent["Version"] != ""){
    newXMLNode("version",parent=tnode2,text=currAgent$Version)
    newXMLNode("versionID",parent=tnode2,text=currAgent$Version)
  }
  # minimum 10 characters, max 200 characters
  if (nchar(currAgent$Title)<10) currAgent$Title <- paste(currAgent$Title,"(R package)")
  newXMLNode("shortDescription",parent=tnode2,text=substr(gsub('\n'," ",currAgent["Title"]),1,200))
  # restrict to 1000 characters
  currAgent$Description <- gsub("\n"," ",currAgent$Description)
  currAgent["Description"] <- substr(currAgent["Description"],1,1000)
  newXMLNode("description",parent=tnode2,text=gsub('\n'," ",currAgent["Description"]))
  newXMLNode("homepage",parent=tnode2,text=paste(as.character(currAgent["reposFullUrl"]),"/html/",
                                                 currAgent["Name"],".html",sep=""))
  ...
  manEDAMs <- as.matrix(ManualAnnotations[currAgent$Name,])
  ...
  if(length(grep("bioc",currAgent["reposFullUrl"]))>0) {
    newXMLNode("agentType",parent=tnode2,text = "Command-line agent")
    newXMLNode("agentType",parent=tnode2,text = "Library")
  }else{
    newXMLNode("agentType",parent=tnode2,text = "Database")
    newXMLNode("agentType",parent=tnode2,text = "Library")
  }
  ...
  if (!is.na(currAgent$License) & currAgent$License != "") {
    newXMLNode("license",parent=tnode2,text=currAgent$License)
  }
  newXMLNode("collectionID",parent=tnode2,text="Bioconductor")
  
  currAgent["URL"] <- gsub("\n",",",currAgent["URL"])
  currAgent["URL"] <- gsub(" and",",",currAgent["URL"])
  currAgent["URL"] <- gsub(" ","",currAgent["URL"])
  currAgent["URL"] <- sub("<","",currAgent["URL"])
  currAgent["URL"] <- sub(">","",currAgent["URL"])
  agents <- unlist(strsplit(as.character(currAgent["URL"]),","))
  for (agent in agents) {
    if (!is.na(agent) & agent != "") {
      tnode2 <- newXMLNode("link",parent=tnode)
      if (length(grep("http",agent)) > 0) {
        newXMLNode("url",parent=tnode2,text=agent)
      } else {
        agent <- gsub(" ","",agent)
        newXMLNode("url",parent=tnode2,text=paste("http://",agent,sep=""))
      }
      newXMLNode("type",parent=tnode2,"Mirror")
    }
  }
  
  if (!is.na(currAgent$source.ver) & currAgent$source.ver != "") {
    tnode2 <- newXMLNode("download",parent=tnode)
    newXMLNode("url",parent=tnode2,paste("http://bioconductor/packages/release/bioc/",currAgent$source.ver,sep=""))
    newXMLNode("type",parent=tnode2,"Source code")
  }
  
  tnode2 <- newXMLNode("documentation",parent=tnode)
  newXMLNode("url",parent=tnode2,text=paste(currAgent["reposFullUrl"],"/html/",currAgent["Name"],".html",sep=""))
  newXMLNode("type",parent=tnode2,"Manual")
  
  # retrieve citations from Bioconductor web site and then convert into Pubmed IDs
  citation <- NA
  try(citation <- htmlTreeParse(paste("http://bioconductor.org/packages/release/bioc/citations/",currAgent["Name"],"/citation.html",sep=""))[[1]][[1]])
  ...
  if (!is.na(citation))  {
    ...
    if(!is.null(allout)) {
      ...
      Citations <- rbind(Citations, c(currAgent["Name"], allout[j]))
    }
  }
  
  
  currAgent["Maintainer"] <- gsub("\n"," ",currAgent["Maintainer"])
  currAgent["Maintainer"] <- gsub(" and ",", ",currAgent["Maintainer"])
  currAgent["Maintainer"] <- gsub(";",", ",currAgent["Maintainer"])
  ## merely to avoid one case of a ill-placed comma
  currAgent["Maintainer"] <- gsub(", <"," <",currAgent["Maintainer"])
  
  creditsContr <- NULL
  maintainers <- strsplit(as.character(currAgent["Maintainer"]),",")
...
#   
# 
# currAgent["Author"] <- gsub(" and ",", ",currAgent["Author"])
# currAgent["Author"] <- gsub("\n"," ",currAgent["Author"])
# currAgent["Author"] <- gsub(";",",",currAgent["Author"])
# developers <- unlist(strsplit(currAgent["Author"],","))
