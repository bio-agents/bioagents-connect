...
Source:
 ...
 and a redundant copy is held on
  https://github.com/bio-agents/bioagentsConnect.git/DebianMed/edam.sh
...
-- (
  SELECT DISTINCT
         ...
         p.source, p.homepage, p.license as license, p.blend as blend,
          ...
          bioagents.entry  as "bio.agents",
          omicagents.entry as "OMICagents",
    ...
    LEFT OUTER JOIN registry bioagents  ON p.source = bioagents.source   AND bioagents.name  = 'bio.agents'
    LEFT OUTER JOIN registry omicagents ON p.source = omicagents.source  AND omicagents.name = 'OMICagents'
