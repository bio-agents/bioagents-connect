    ...
    for item in data:
        ...
        if package == package_source:
            if bioagents is None:
                ...
                if p.is_dir():
                    rootLogger.warning(
                        f"package '{package_source}' has no bio.agents ref but bio.agents has a cognate one, skipping."
                    )
                    continue
                else:
                    rootLogger.warning(f"package '{package_source}' has no bio.agents ref, skipping.")
                    continue
            else:
                ...
                if not p.is_dir():
                    rootLogger.warning(
                        f"package '{package_source}' has a bioagents ref ('{bioagents}') but no folder exists, skipping."
                    )
                    continue
        else:
            rootLogger.warning(
            ...
            continue
        rootLogger.info(
            f"processing package '{package_source}' with bioagents ref ('{bioagents}')."
        )
